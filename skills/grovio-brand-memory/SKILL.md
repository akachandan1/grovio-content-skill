...
| Task | Input | Output |
|---|---|---|
| Save brand profile | Brand intelligence from grovio-content session | Persisted brand memory file |
...
```
/grovio-brand-memory save [brand_name] [--from-session]
```

When `--from-session` is passed, extracts the brand intelligence from the current conversation's grovio-content run and saves it.
...
## Campaign History
[Log of content campaigns run through Grovio system]
...
```
/grovio-brand-memory load [brand_name_or_url]
```
...
When brand memory exists, the grovio-content master orchestrator should:
...
```
/grovio-brand-memory update [brand_name] [section] [new_information]
```
...
**Campaign logging (automatic after each grovio-content run):**
```
...
```
/grovio-brand-memory compare [brand_a] [brand_b]
```
...
The grovio-content master orchestrator checks for brand memory automatically:
